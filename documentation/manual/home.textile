...
play-ebean module adds Ebean:http://http://www.avaje.org ORM to Play! Framework as an alternative to Hibernate. Currently we use this module in two applications  of my company, but I don't know if other has tested it. Since Ebean uses JPA annotations as Hibernate no change in model shuould be done in general.
...
bc. Ebean has it's own query language. Prior to this decision JPQL (the JPA query language)
was investigated to see if it would meet the desired goals of Ebean and it did not.

bc. Specifically I wanted to support “Partial Objects” via the query language and it is difficult
